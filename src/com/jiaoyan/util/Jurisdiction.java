...
public class Jurisdiction {
	...
	public static boolean hasJurisdiction(String menuUrl){
		...
		List<MenuTree> menuList = (List<MenuTree>)session.getAttribute(USERNAME + Const.SESSION_allmenuList); //获取菜单列表
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static boolean readMenu(List<MenuTree> menuList,String menuUrl,Session session,String USERNAME){
		for(int i=0;i<menuList.size();i++){
			if(menuList.get(i).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
				if(!menuList.get(i).isHasMenu()){				//判断有无此菜单权限
					...
					String[] adds=Tools.str2StrArray(map.get("adds"));
					for (String add : adds) {
					   map.put("add", (RightsHelper.testRights(add, MENU_ID)) || isAdmin?"1":"0");
					   if(RightsHelper.testRights(add, MENU_ID)){
						   break;
					   }
					}
					String[] dels=Tools.str2StrArray(map.get("dels"));
					for (String del : dels) {
					   map.put("del", (RightsHelper.testRights(del, MENU_ID)) || isAdmin?"1":"0");
					   if(RightsHelper.testRights(del, MENU_ID)){
						   break;
					   }
					}
					String[] edits=Tools.str2StrArray(map.get("edits"));
					for (String edit : edits) {
					   map.put("edit", (RightsHelper.testRights(edit, MENU_ID)) || isAdmin?"1":"0");
					   if(RightsHelper.testRights(edit, MENU_ID)){
						   break;
					   }
					}
					String[] chas=Tools.str2StrArray(map.get("chas"));
					for (String cha : chas) {
					   map.put("cha", (RightsHelper.testRights(cha, MENU_ID)) || isAdmin?"1":"0");
					   if(RightsHelper.testRights(cha, MENU_ID)){
						   break;
					   }
					}
					 //map.put("cha", "1");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static boolean readMenuButton(List<MenuTree> menuList,String menuUrl,Session session,String USERNAME, String type){
		for(int i=0;i<menuList.size();i++){
			if(menuList.get(i).getMENU_URL().split(".do")[0].equals(menuUrl.split(".do")[0])){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
				if(!menuList.get(i).isHasMenu()){				//判断有无此菜单权限
					return false;
					...
					if("add".equals(type)){
						return ((RightsHelper.testRights(map.get("add"), MENU_ID)) || isAdmin);
					}else if("del".equals(type)){
						return ((RightsHelper.testRights(map.get("del"), MENU_ID)) || isAdmin);
					}else if("edit".equals(type)){
						return ((RightsHelper.testRights(map.get("edit"), MENU_ID)) || isAdmin);
					}else if("cha".equals(type)){
						return ((RightsHelper.testRights(map.get("cha"), MENU_ID)) || isAdmin);
					}
				}
			}else{
